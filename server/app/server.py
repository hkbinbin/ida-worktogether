    ...
    def handle(self):
        ...
        try:
            while True:
                ...
                if not data:
                    break
                datas:list[bytes] = data.split(b'\r\n')
                for json_data in datas:
                    print(json_data)
                    # print(json_data)
                    if len(json_data) == 0: # for split = b'' 
                        continue
                    is_boradcast = process_buffer_from_client(json_data,self.request)
                    if is_boradcast == True:
                        conn_manager.broadcast(json_data, exclude_request=exclude_sockets,self_request=self.request)
...
class MyTCPServer(socketserver.ThreadingTCPServer):
    daemon_threads = True
    allow_reuse_address = False  # prevent "address already in use" error
