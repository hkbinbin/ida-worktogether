...
import config_
import general_func_ as general
@dataclass
class VarInfo:
    # var type
    typeinfo: str
...
class LocalvarHexrayHooks(ida_hexrays.Hexrays_Hooks):
    ...
    def func_printed(self, cfunc):
        ...
        if self.mutex_func_printed == False:
            ...
            sub_socket = general.sub_connect_server()
                ...
                while True: # cycle for json_list
                    ...
                    while True: # cycle for full json data
                        ...
                        if data.endswith(b"E0@F"):
                            ...
                            sub_socket.sendall("ACK".encode()) # send ACK
                            json_data = b""
                            break
    ...
    def lvar_name_changed(self, vu, v, name, is_user_name):
        ...
        for i, lv in enumerate(lvars):
            # check arg var
            print(f"[LOG] var/i: {lv.name}, {i}")
            if lv.is_arg_var:
                # check mismatch in cache
                print(f"[LOG] lv_name: {lv.name} vs cached_name: {cached[i].lvarname}")
